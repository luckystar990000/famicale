...
## v2 計画 (2026-07-19 起票、 妻の v1 フィードバック起点)

実ユーザー (妻) の一刀両断 4 点 (詳細はメモリ [[project-famicale-real-usage]]) を主軸にする。 「ホーム = カウントダウン (待つ・忘れない)」 は維持し、 「計画する」 視点を足す二本立て。

1. **カレンダービュー (最優先候補)**: 月表示グリッド + 日別ドット + 日タップで予定リスト。 「予定を入れる時に既存予定が見えない」 の解消。 新規イベントフォームからの俯瞰導線も検討
2. **繰り返し予定**: まず毎週 (曜日指定) から。 スキーマに repeat ルール、 カウントダウンは「次回」基準。 隔週・毎月は様子見
3. **「誰の」予定の一級市民化**: 家族メンバー登録 (名前 + 色) → ホームカードに色付き人物チップ。 タグとの関係を整理 (人タグを色付き特別扱いにする案が最小)
4. **イベント語彙の文脈対応**: 「開催まで」 をイベント種別で出し分け or 中立な「あと N 日」 に。 2026-05-21 の誕生日ラベル問題と同根、 まとめて解消

未着手。 スコープと着手順はユーザーと相談して決める。
