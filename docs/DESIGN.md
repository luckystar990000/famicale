...
- **Body** (padding `10px 16px`): 1 行目に日付テキスト (14px / 500 / dateColor、 1 行 ellipsis)。 visitDate set 時は「`5/1 〜 9/30`」 (会期 = startDate/endDate)、 not set 時は `formatDateLabel(schedule, status)` の dateText。 2 行目以降に notes (13px / subTextColor / 2 行 line-clamp / pre-wrap)。
...
- 会期補足 (`会期: ○月○日 〜 ○月○日`) — 単独表示はしない (visitDate set 時は Body の日付テキストが会期そのもの)
