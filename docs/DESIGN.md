...
カードを **Header / Body / Footer + Gauge** の 4 ブロックに分けて構造化する。 ガラスカード、押下フィードバック (`press-feedback`)、角丸 `border-radius: 27px`。

- **Header** (padding `14px 16px 6px`): 左にタイトル (16px / 600 / 1 行 ellipsis)、 右に countdown バッジ (`statusBadge` の色)。 cancelled なら taittle を line-through + badge を「中止」 赤系。 ending-soon / upcoming-soon の badge は `badge-pulse` で明滅。
- **Body** (padding `0 16px 10px`): 1 行目に「行く日」 pill (visitDate set 時) + 日付テキスト (`dateText`)、 2 行目に notes (2 行 line-clamp、 pre-wrap)。 notes が無ければ 1 行のみ。
...
- **Gauge** (最下端 4px): カード末尾、 アクセント色の左 or 右寄り fill。 ゲージ方向は本ドキュメント上部「ゲージ」 セクション参照。

**カード外に出した情報** (詳細画面でしか見せない):
- StatusDot (左の状態丸印) — 削除。 状態色は gauge + countdown badge で訴求
...
**Why H/B/F 構造化** (2026-05-24 ユーザー指示): 旧構造は 1 つの flex 横並びにあらゆる情報を詰め込んでいて、 ぱっと見の情報密度が高かった。 「タイトル + countdown」 が一番大事 = Header、 「いつ / 何の予定」 が次 = Body、 「タグや進捗」 が補助 = Footer、 と意味階層を視覚分離することで「一覧で何を見るか」 が明確になる。
