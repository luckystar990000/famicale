...
- TabBar (ボトムタブ) も同様: 半透明白 (0.65)、線なし。
...
- カード: `border-radius: 14px`、半透明白 + blur、内側上端ハイライト、薄縁取り、軽い落ち影、`overflow: hidden`
- **セパレーター (区切り線)**:
  ...
  - 位置: **左 16px / 右 16px インデント** (左右対称、角丸を貫通せず自然に止まる)
  - 各 ListRow には `border-top` を持たせない (擬似要素任せ)
  - 最初の行 (`:first-child`) には線を描画しない
- 行高さ: `min-height: 44px` (タップターゲット)
...
- アクティブ: tint ベタ + 白文字 + 600 ウェイト、 非アクティブ: 透明 + 0.5px separator 枠 + label 色
...
iOS 風のボトムシートモーダル。 詳細画面の各フィールドのインライン編集に使う。

- 構造: backdrop (半透明黒) + シート (下から slide-up、 角丸上端 16px、 背景 `var(--bg-grouped)`、 セーフエリア対応、 最大高さ 85dvh)
- ヘッダ: 3 段グリッド「キャンセル / 中央タイトル / 保存」、 下端 `0.5px var(--separator)`
- 中身: scroll 可能領域、 padding 16px
...
| プライマリ CTA (画面下の大きい確定) | フル幅 / 2:1 比率の **角丸ボタン** (radius 10-12)、tint ベタ塗り + シャドウ | SharePage の「発行する」 |
...
- **NavBar / TabBar の境界に再度 backdrop-filter blur を導入しようとしない**。iOS 26 Safari で動かないことを確認済み (sticky 親に直接 / absolute child パターン両方試行済み)。現状の 0.85 半透明白 + 0.5px 線で固定。詳細は [[project-famicale-environment]]。
